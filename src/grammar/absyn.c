...
#define ENOUGH                                                                 \
  36 // should be enough to hold constructor names and small expressions

extern NodeST
    *ST; // Symbol Table with all allocated nodes (for cleanup on parse error)
...
ExprList *addExprToList(Expr *expression, ExprList *list) {
  ...
  Expr **components = list->components;

  if (size >= space) { // grow (no need to shrink as list always grows)
    space = 2 * size + 1;
    list->components = (Expr **)realloc(components, space * sizeof(Expr *));
    list->space = space;
  }
...
Rules *addRuleToList(Rule *statement, Rules *list) {
  int size = list->size;
  int space = list->space;
  Rule **components = list->components;

  if (size >= space) { // grow (no need to shrink as list always grows)
    space = 2 * size + 1;
    list->components = (Rule **)realloc(components, space * sizeof(Rule *));
    list->space = space;
  }
...
void freeNode(void *node, NodeType type) {
  KeyT key; // will need it to take node out of ST later
...
/*
  Free the abstract syntax subtree with root node. Also takes the corresponding
  nodes out of the ST.
*/
...
char *intToString(long long int a) {
  ...
  char *str =
      (char *)malloc(sizeof(char) * ((int)ceil(log10(abs + 1)) +
                                     2)); // +2: sign and  NULL terminator
...
/*
   String representation of Units. Always encapsulates string literals in
   malloc'ed char arrays.
*/
...
/*
  String representation of Ids. We avoid simply returning A->name to enforce the
  philosophy that all strings are owned by this module, so we can (and should)
  free the returned strings.
*/
char *idToString(const Id *A) {
  char *str =
      (char *)malloc(sizeof(char) * (strlen(A->name) + 1)); // NULL terminator
...
*/
char *exprToString(const Expr *E) {
  char *str = (char *)malloc(sizeof(char) * ENOUGH);
  ...
  switch (E->type) {
  case (UNION):;
    ExprList *elist = (ExprList *)E->component;
...
*/
char *exprListToString(const ExprList *Elist) {
  int size = Elist->size;
  int length = 0;
  Expr **subexps = Elist->components;
  char **substrs = (char **)malloc(sizeof(char *) * size);

  for (int i = 0; i < size;
       i++) { // get string representation of subexpressions
  ...
  length +=
      2 * (size - 1) + 1; // for ", " between subexpressions and NULL terminator
...
char *ruleToString(const Rule *S) {
  Id *var = S->variable;
  Expr *exp = S->expression;
  char *varstr = var->toString(var);
  char *expstr = exp->toString(exp);
  char *str = (char *)malloc(sizeof(char) * (strlen(varstr) + strlen(expstr) +
                                             4)); // NULL terminator and " = "
...
char *rulesToString(const Rules *Slist) {
  int size = Slist->size;
  int length = 0;
  Rule **substms = Slist->components;
  char **substrs = (char **)malloc(sizeof(char *) * size);
  ...
  length +=
      2 * (size - 1) + 1; // for ", " between substatements and NULL terminator
  char *str = (char *)malloc(sizeof(char) * length);
  sprintf(str, "%s", substrs[0]); // there is always a first element
  free(substrs[0]);
...
char *errorToString(const Error *error) {
  ...
  char *str = (char *)malloc(sizeof(char) *
                             (strlen(error->message) + strlen(line) + ENOUGH));
  if (error->type == LEXER) {
...
char *specToString(const Spec *grammar) {
  if (grammar->type == ISERROR) {
    Error *E = (Error *)grammar->component;
    return E->toString(E);
  } else {
    Rules *Slist = (Rules *)grammar->component;
...
/*
   Json representation of Units. Always encapsulates json literals in malloc'ed
   char arrays.
*/
// DONE:
...
/*
  Json representation of Ids. We avoid simply returning A->name to enforce the
  philosophy that all jsons are owned by this module, so we can (and should)
  free the returned jsons.
*/
// FIXME: fix below
...
char *exprToJson(const Expr *E) {
  ...
  case (EPSILON):
    ...
    return strdup("{ \"type\": \"unit\", \"unit\": \"Epsilon\" }");
  case (Z):
    // sprintf(str, "{ \"id\": \"Z\" }"); // Not sure about this one (FIXME:
    // Make it an alias)
    return strdup("{ \"type\": \"id\", \"id\": \"Z\" }");
  case (ID):;
    // free(str);
...
#define JSONPATT_UNION "{ \"type\": \"op\", \"op\": \"Union\", \"param\": %s }"
    ...
    str = (char *)realloc(
        str, sizeof(char) * (strlen(subexps) + strlen(JSONPATT_UNION) + 1));
...
#define JSONPATT_PROD "{ \"type\": \"op\", \"op\": \"Prod\", \"param\": %s }"
    ...
    str = (char *)realloc(
        str, sizeof(char) * (strlen(subexps) + strlen(JSONPATT_PROD) + 1));
...
#define JSONPATT_SUBST "{ \"type\": \"op\", \"op\": \"Subst\", \"param\": %s }"
    ...
    subexps = elist->toJson(elist);
    str = (char *)realloc(
        str, sizeof(char) * (strlen(subexps) + strlen(JSONPATT_SUBST) + 1));
...
#define JSONPATT_SET "{ \"type\": \"op\", \"op\": \"Set\", \"param\": [%s]%s }"
    ...
    str = (char *)realloc(str, sizeof(char) * (strlen(subexp) + strlen(rest) +
                                               strlen(JSONPATT_SET) + 1));
    sprintf(str, JSONPATT_SET, subexp, rest);
    free(rest);
    free(subexp);
    return str;
  case (POWERSET):;
#define JSONPATT_POWERSET                                                      \
  "{ \"type\": \"op\", \"op\": \"PowerSet\", \"param\": [%s]%s }"
    e = (Expr *)E->component;
    rest = restrictionToJson(E->restriction, E->limit);
    subexp = e->toJson(e);
    str = (char *)realloc(str, sizeof(char) * (strlen(subexp) + strlen(rest) +
                                               strlen(JSONPATT_POWERSET) + 1));
    sprintf(str, JSONPATT_POWERSET, subexp, rest);
    free(rest);
    free(subexp);
    return str;
  case (SEQUENCE):;
#define JSONPATT_SEQUENCE                                                      \
  "{ \"type\": \"op\", \"op\": \"Sequence\", \"param\": [%s]%s }"
    e = (Expr *)E->component;
    rest = restrictionToJson(E->restriction, E->limit);
    subexp = e->toJson(e);
    str = (char *)realloc(str, sizeof(char) * (strlen(subexp) + strlen(rest) +
                                               strlen(JSONPATT_SEQUENCE) + 1));
    sprintf(str, JSONPATT_SEQUENCE, subexp, rest);
    free(rest);
    free(subexp);
    return str;
  case (CYCLE):;
#define JSONPATT_CYCLE                                                         \
  "{ \"type\": \"op\", \"op\": \"Cycle\", \"param\": [%s]%s }"
    e = (Expr *)E->component;
    rest = restrictionToJson(E->restriction, E->limit);
    subexp = e->toJson(e);
    str = (char *)realloc(str, sizeof(char) * (strlen(subexp) + strlen(rest) +
                                               strlen(JSONPATT_CYCLE) + 1));
...
*/
// FIXME: fix below
char *exprListToJson(const ExprList *Elist) {
  int size = Elist->size;
  int length = 0;
  Expr **subexps = Elist->components;
  char **substrs = (char **)malloc(sizeof(char *) * size);
...
// DONE:
char *rulesToJson(const Rules *Slist) {
  int size = Slist->size;
  int length = 0;
  Rule **substms = Slist->components;
  char **substrs = (char **)malloc(sizeof(char *) * size);
...
char *errorToJson(const Error *error) {
  ...
  char *str = (char *)malloc(
      sizeof(char) * (strlen(error->message) + strlen(line) + 2 * ENOUGH));
  if (error->type == LEXER) {
    sprintf(str,
            "{\n  \"type\": \"error\",\n  \"source\": \"lexer\",\n  \"line\": "
            "%s\n  \"msg\": \"%s\"\n}",
...
// DONE:
char *specToJson(const Spec *grammar) {
  if (grammar->type == ISERROR) {
    Error *E = (Error *)grammar->component;
    return E->toJson(E);
  } else {
...
ExprList *newExprList(Expr *expression) {
  ExprList *Elist = malloc(sizeof(ExprList));
  Expr **components = (Expr **)malloc(sizeof(Expr *));
  components[0] = expression;
  Elist->components = components;
  Elist->size = 1;
  Elist->space = 1;
  Elist->toString = &exprListToString;
  Elist->toJson = &exprListToJson;
  Elist->key = addNode(Elist, EXPLIST_N, ST);
  return Elist;
}

Rule *newRule(Id *variable, Expr *expression) {
  Rule *S = malloc(sizeof(Rule));
  S->variable = variable;
  S->expression = expression;
  S->toString = &ruleToString;
  S->toJson = &ruleToJson;
...
Rules *newRules(Rule *statement) {
  Rules *Slist = malloc(sizeof(Rules));
  Rule **components = (Rule **)malloc(sizeof(Rule *));
  components[0] = statement;
  Slist->components = components;
  Slist->size = 1;
  Slist->space = 1;
  Slist->toString = &rulesToString;
  Slist->toJson = &rulesToJson;
...
Spec *newSpec(void *component, SpecType type) {
  Spec *G = malloc(sizeof(Spec));
  G->component = component;
  G->type = type;
  G->toString = &specToString;
  G->toJson = &specToJson;
